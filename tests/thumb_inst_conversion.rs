...
fn get_result(result: Option<u32>) -> u32 {
    match result {
        Some(x) => x,
        None    => panic!("Must be an instruction")
    }
}
...
mod test {
    ...
    fn convert_soft_int() {
        ...
        let converted: Option<u32>  = proccore::translate_thumb(thumbinst, proccore::decode_thumb(thumbinst));
        assert_eq!(arminst, get_result(converted));
    }
    ...
    #[test]
    fn convert_long_branch_lnk() {
        let thumbinst: u16  = 0b1111101010101010;
        let converted: Option<u32>  = proccore::translate_thumb(thumbinst, proccore::decode_thumb(thumbinst));
